...
const Attendance = () => {
	...
	const [ userData, setUserData ] = useState({ attendance: 'timein', date: date_now, username: session && !session.isAdmin ? session.username : '', timein: time_check, timeout: time_check })
	...
	const HANDLE_SUBMIT = async (e) => {
		...
		if ( attendance === 'timein' ) {
			...
			await axios.post('/api/attendance/timein', {
				date, username, time: time_check
			}).then((response) => {
				setUseSpinner(false)
				setShowSuc(true)
				...
				if ( error.response.data.usernameError ) {
					setUseSpinner(false), setShowSuc(false), setShowErr(true), setMessage(error.response.data.usernameError)
					return
				}

				if ( error.response.data.dateError ) {
					setUseSpinner(false), setShowSuc(false),setShowErr(true), setMessage(error.response.data.dateError)
					return
				}
			})
		} else {
			...
			await axios.post('/api/attendance/timeout', {
				date, username, time: time_check
			}).then((response) => {
				setUseSpinner(false)
				setShowSuc(true)
				...
				if ( error.response.data.usernameError ) {
					setUseSpinner(false)
					setShowSuc(false), setShowErr(true), setMessage(error.response.data.usernameError)
					return
				}

				if ( error.response.data.dateError ) {
					setUseSpinner(false)
					setShowSuc(false), setShowErr(true), setMessage(error.response.data.dateError)
					return
				}
			})
		}
			...
			<Wrapper>
				...
				<Form onSubmit={ HANDLE_SUBMIT }>
					...
					<InputField>
						...
						<input type="text" placeholder="Username" name="username" onChange={ HANDLE_INPUTS } defaultValue={ session && !session.isAdmin ? session.username : '' } readOnly={ session && session.isAdmin ? false : true } />
