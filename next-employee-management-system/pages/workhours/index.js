...
const WorkHours = () => {
	...
	const { data, error } = useSWR('/api/workhours', fetcher, { refreshInterval: 1000 })
	...
	const HANDLE_EDIT = ( _id, timein, timeout ) => {
		...
		dispatch({ type: 'WORKHOURS_EDIT_DATA', payload: { _id, timein, timeout } })
	}
	...
	const HANDLE_DELETE = async () => {
		await axios.post('/api/workhours/delete', {
						...
						<tbody>
							{
								data && data.map(data => (
									<tr key={ data._id }>
										<td><input type="time" value={ data.timein } readOnly={ true } /></td>
										<td><input type="time" value={ data.timeout } readOnly={ true } /></td>

										<td>
											<button onClick={ () => HANDLE_EDIT(data._id, data.timein, data.timeout ) }>
												<EditIcon />
											</button>
