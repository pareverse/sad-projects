...
const Add = () => {
	...
	const date_now = name[date.getMonth()] + ' ' + date.getDate() + ', ' + date.getFullYear()
	...
	const [ userData, setUserData ] = useState({ id: employee_generate_id, username: '', firstname: '', lastname: '', address: '', birthdate: '', contact: '', gender: '', position: '', timein: '', timeout: '', schedule: '', joined: date_now.toString() })
	const { id, username, firstname, lastname, address, birthdate, contact, gender, position, timein, timeout, schedule, joined } = userData
	...
	const HANDLE_SUBMIT = async (e) => {
		...
		await axios.post('/api/employees/add', {
			id, username, firstname, lastname, address, contact, birthdate, gender, position, timein: schedule.slice(0, 5), timeout: schedule.slice(8, 13), schedule, joined
		}).then((response) => {
			router.push('/employees')
			...
			<Wrapper>
				...
				<Items>
					<form onSubmit={ HANDLE_SUBMIT }>
						<input type="text" name="id" value={ employee_generate_id } onChange={ HANDLE_INPUTS } disabled />
						<input type="text" placeholder="Username" name="username" onChange={ HANDLE_INPUTS } />
						<input type="text" placeholder="Firstname" name="firstname" onChange={ HANDLE_INPUTS } />
						<input type="text" placeholder="Lastname" name="lastname" onChange={ HANDLE_INPUTS } />
						<input type="text" placeholder="Address" name="address" onChange={ HANDLE_INPUTS } />
						<input type="text" placeholder="Contact" name="contact" onChange={ HANDLE_INPUTS } />
						<input type="date" placeholder="Birthdate" name="birthdate" onChange={ HANDLE_INPUTS } />
						...
						<select name="position" onChange={ HANDLE_INPUTS }>
							...
							{
								position_list && position_list.map(data => (
									<option value={ data.position } key={ data._id}>{ data.position }</option>
								))
							}
						</select>
						...
						<select name="schedule" onChange={ HANDLE_INPUTS }>
							...
							{
								workhours_list && workhours_list.map(data => (
									<option value={ data.timein + ' - ' + data.timeout } key={ data._id}>{ data.timein + ' - ' + data.timeout }</option>
								))
							}
						</select>
