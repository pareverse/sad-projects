...
const Attendance = () => {
	...
	const { data: employees, error: employees_error } = useSWR('/api/employees', fetcher, { refreshInterval: 1000 })
	...
	return (
		<Container>
			<Wrapper>
				...
				<Items>
					<Link href="/" onClick={ () => dispatch({ type: 'SIDEBAR', payload: false }) }>
									...
									attendance && attendance.map(data => (
										employees && employees.map(emp => (
											...
											<tr key={ data._id }>
												<td>{ data.date }</td>
												<td>{ emp.id }</td>
												<td>{ emp.firstname.toUpperCase() + ' ' + emp.lastname.toUpperCase() }</td>
												
												<td>
													<input type="time" value={ data.att_timein } readOnly={ true } />
													{ data.late >= 1 ? <Late>LATE - { data.late } hr/s</Late> : <Ontime>ONTIME</Ontime> }
												</td>

												<td>
													<input type="time" value={ data.att_timeout } readOnly={ true } />
													{ data.overtime >= 1 ? <Overtime>OT - { data.overtime } hr/s</Overtime> : <Ontime>ONTIME</Ontime> }
												</td>
							...
							<tbody>
								{
									attendance && attendance.map(data => (
										employees && employees.map(emp => (
											session && session.username === data.username && data.username === emp.username && 
											<tr key={ data._id }>
												<td>{ data.date }</td>
